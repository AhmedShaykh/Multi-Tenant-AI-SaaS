...
const DocumentsPage = () => {
    ...
    const handleAnalyze = async (documentId: string) => {
        ...
        try {
            ...
            toast.success(`${analysisTypeLabel || "Document"} Analysis Completed Successfully!`);
    ...
    const analyzedCount = documents.filter((document) => document.aiSummary).length;

    const pendingCount = documents.filter((document) => !document.aiSummary,).length;

    const totalSize = documents.reduce((total, document) => total + (document.fileSize || 0), 0);
